"""First-class OpenClaw gateway integration for loom."""
...
def build_openclaw_config(
    *,
    ...
    """Build a direct loom OpenClaw config from CLI args and optional JSON."""
...
def config_from_environment() -> OpenClawConfig:
    """Allow non-web/manual runs to opt into loom OpenClaw directly."""
    headers: dict[str, str] = {}
    raw_headers = os.environ.get("LOOM_OPENCLAW_HEADERS", "")
    for item in [x.strip() for x in raw_headers.splitlines() if x.strip()]:
        key, value = _parse_header(item)
        headers[key] = value
    url = os.environ.get("LOOM_OPENCLAW_URL", "")
    return OpenClawConfig(
        enabled=os.environ.get("LOOM_OPENCLAW", "") == "1" or bool(url),
        url=url,
        headers=headers,
        timeout_ms=int(os.environ.get("LOOM_OPENCLAW_TIMEOUT_MS", "10000")),
        debug=os.environ.get("LOOM_OPENCLAW_DEBUG", "") == "1",
        hook=os.environ.get("LOOM_OPENCLAW_HOOK", "wake"),
        wake_mode=os.environ.get("LOOM_OPENCLAW_WAKE_MODE", "now"),
        agent_name=os.environ.get("LOOM_OPENCLAW_AGENT_NAME", "loom"),
        agent_id=os.environ.get("LOOM_OPENCLAW_AGENT_ID", ""),
        deliver=os.environ.get("LOOM_OPENCLAW_DELIVER", "1") != "0",
        channel=os.environ.get("LOOM_OPENCLAW_CHANNEL", ""),
        to=os.environ.get("LOOM_OPENCLAW_TO", ""),
    )
