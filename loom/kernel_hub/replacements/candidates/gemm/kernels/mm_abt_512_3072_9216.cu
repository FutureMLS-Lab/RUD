// mm_abt_512x3072x9216.cu — Hand-tuned bf16 ABt GEMM for M=512, N=3072, K=9216
//
// C[m,n] = sum_k A[m,k] * B[n,k]   (B row-major N×K, matching F.linear weight)
//
// Adapted from mm_512_3072_7680.cu (the proven faster-than-cuBLAS kernel for 512×3072×7680)
// with K changed to 9216.
//
// Config:
//   Tile:   128M × 96N × 64K
//   Grid:   {TILES_N, TILES_M, 1} = {32, 4, 1} = 128 blocks
...
//   K-iterations: 9216/64 = 144, rounds: 144/6 = 24
...
static constexpr int GM = 512, GN = 3072, GK = 9216;
...
static constexpr int NUM_ROUNDS = TILES_K / QSIZE;  // 144/6 = 24
...
// Globals struct: holds TMA descriptors + dims.
// Passed as __grid_constant__ to the kernel (single argument, like TK kernels).
struct Globals512x3072x9216 {
    CUtensorMap tmaA;
    CUtensorMap tmaB;
    CUtensorMap tmaC;
    bf16*       d_C;
...
void gemm_512x3072x9216_kernel(const __grid_constant__ Globals512x3072x9216 G) {
    ...
    // L2-friendly swizzle: group TILES_M adjacent M-tiles per column
    // to maximize B-tile reuse within an L2 sector.
    // Linear block ID → (block_m, block_n) with M-tiles contiguous.
    ...
    // TMA store output: thread 0 writes both 64-row halves of the 128×96 output tile
    if (threadIdx.x == 0) {
        asm volatile("fence.proxy.async.shared::cta;\n" ::: "memory");
...
// ---------------------------------------------------------------------------
// Host-side helpers: create TMA tensor maps and launch kernel
// ---------------------------------------------------------------------------
...
static void create_tensor_map_load(CUtensorMap *tma_map, bf16* gmem_ptr,
    ...
    CUresult r = cuTensorMapEncodeTiled(tma_map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2, (void*)gmem_ptr,
        shape, stride + 1, box, bstride,
        CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_128B,
        CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
    if (r != CUDA_SUCCESS) { fprintf(stderr, "[mm_abt_512x3072x9216] TMA load map failed: %d\n", (int)r); }
}
...
static void create_tensor_map_store(CUtensorMap *tma_map, bf16* gmem_ptr,
    ...
    CUresult r = cuTensorMapEncodeTiled(tma_map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 2, (void*)gmem_ptr,
        ...
        CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
    if (r != CUDA_SUCCESS) { fprintf(stderr, "[mm_abt_512x3072x9216] TMA store map failed: %d\n", (int)r); }
}

extern "C" int gemm_512x3072x9216_globals_size() {
    return (int)sizeof(Globals512x3072x9216);
}

extern "C" void gemm_512x3072x9216_make_globals(
    void* out_buf,
    void* d_A, void* d_Bt, void* d_C,
    int M, int N, int K
) {
    Globals512x3072x9216 G;
    int tiles_m = M / BM;
    int tiles_n = N / BN;
    int tiles_k = K / BK;
...
extern "C" void gemm_512x3072x9216_grid_dims(int M, int N, int K, int* x, int* y, int* z) {
    *x = (M / BM) * (N / BN);  // TILES_M * TILES_N = 4 * 32 = 128 (1D grid)
    *y = 1;
    *z = 1;
}

extern "C" int gemm_512x3072x9216_block_dim() {
    return NUM_THREADS;
}

extern "C" int gemm_512x3072x9216_shmem_bytes() {
    return (int)sizeof(SMem);
}

extern "C" int gemm_512x3072x9216_num_tma_descriptors() { return 3; }

extern "C" void gemm_512x3072x9216_describe_tma_descriptors(
    void* d_A, void* d_Bt, void* d_C,
    int M, int N, int K,
    void* out_meta
) {
