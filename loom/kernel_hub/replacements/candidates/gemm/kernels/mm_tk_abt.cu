...
// ABt matmul_template: C = A(M,K) @ Bt(N,K)^T
// B is stored row-major as (N, K) — matches F.linear weight layout directly.
...
using mmt = matmul_template<2, 4, 12>;
...
extern "C" int tk_gemm_globals_size() {
    return (int)sizeof(globals);
}

extern "C" void tk_gemm_make_globals(
    void* out_buf,
...
extern "C" void tk_gemm_grid_dims(int M, int N, int K, int* x, int* y, int* z) {
    *x = mmt::grid(0, 0, 0).x;
    *y = 1;
    *z = 1;
}

extern "C" int tk_gemm_block_dim() {
    return (int)kittens::prototype::detail::NUM_THREADS_v<mmt>;
}

extern "C" int tk_gemm_shmem_bytes() {
    return MAX_SHARED_MEMORY - 1024;
}

extern "C" int tk_gemm_num_tma_descriptors() { return 3; }

extern "C" void tk_gemm_describe_tma_descriptors(
    void* d_A, void* d_Bt, void* d_C,
    int M, int N, int K,
    void* out_meta
) {
    ...
    // Descriptor 1: Bt[N, K] bf16, tile 64×64 (B stored as N×K, transposed)
