...
Use these instructions when you are an external assistant, OpenClaw agent, or remote controller interacting with a running `loom web` instance.

## Mental Model

loom owns the task loop.

- `loom web` exposes a local HTTP API.
...
If OpenClaw runs on another host, connect it to loom using SSH reverse forwarding from the loom machine:
...
On the OpenClaw host, loom is then reachable at:

```text
http://127.0.0.1:8765/
```

OpenClaw can receive loom lifecycle events on its gateway:

```text
http://127.0.0.1:18789/hooks/wake
```

For automatic follow-up work, configure loom to post to:

```text
http://127.0.0.1:18789/hooks/agent
```

Use `/hooks/wake` for lightweight notification. Use `/hooks/agent` when OpenClaw should run an agent turn, inspect loom, and optionally call back into `http://127.0.0.1:8765/api/...`.
...
If `loom web` was started with `--auth-token`, OpenClaw must know the same token. Set it before calling the API:

```bash
LOOM_WEB_AUTH_TOKEN="the-token"
AUTH_HEADER="Authorization: Bearer $LOOM_WEB_AUTH_TOKEN"
...
The Python scripts in `scripts/` automatically read `LOOM_WEB_AUTH_TOKEN` and send `Authorization: Bearer <token>`.

## Project selection (for `/api/project` and `/api/tasks`)

The web API needs a **registry project id** on every `/api/project` and `/api/tasks/...` request unless the server can infer a unique default (`?project=<id>` or header `X-Loom-Project`).
...
1. **`LOOM_PROJECT_ID`** — must match an `id` in the registry (set this when you need a specific repo).
2. **`LOOM_PROJECT_PATH`** — absolute path of a registered project root (resolved and matched against each project’s `path`).
...
```bash
SCRIPT_DIR=/path/to/loom/skills/remote_control/scripts
export LOOM_BASE_URL="http://127.0.0.1:8765"
...
```bash
export LOOM_PROJECT_ID=""  # optional; scripts auto-resolve if unset (see above)

cl_api_scope() {
  local p="$1"
  if [[ -z "${LOOM_PROJECT_ID:-}" ]]; then
    printf '%s%s' "$BASE" "$p"
    return
  fi
  if [[ "$p" == *"?"* ]]; then
    printf '%s%s&project=%s' "$BASE" "$p" "$LOOM_PROJECT_ID"
  else
    printf '%s%s?project=%s' "$BASE" "$p" "$LOOM_PROJECT_ID"
  fi
}
...
Pin the session to the server default project when using **manual** curl and an empty `LOOM_PROJECT_ID`:

```bash
export LOOM_PROJECT_ID="$(
  curl -s "${CURL_AUTH[@]}" "$BASE/api/projects" \
    | python3 -c 'import json,sys; print(json.load(sys.stdin).get("defaultProjectId") or "")'
)"
```

When `LOOM_PROJECT_ID` is empty, `cl_api_scope` does **not** append `project=` — use the Python helpers or set the id explicitly for multi-root servers.
...
- **Header:** `X-Loom-Project: <id>` (query wins if both are set).

Global endpoints such as `POST /api/tmux/send-text` do **not** take `project`. For `GET /api/tmux/sessions`, omit `project` to list every session, or pass `?project=<id>` to list only loom sessions tied to that project id (same naming as the web UI).
...
You may still set `LOOM_PROJECT_ID` or `LOOM_PROJECT_PATH` to force a specific repo when several are registered.

If a request fails, first check that the tunnel is still alive and that `loom web` is running on the local machine.
...
2. List tasks with `GET /api/tasks` (include `?project=<id>` or `X-Loom-Project` when required).
...
List tasks (for **curl**, use `cl_api_scope` with `LOOM_PROJECT_ID` set, or append `?project=$(python3 …/cl_remote_api.py --print-project-id)`; the `scripts/*.py` helpers add `project=` automatically):
  ...
  "tasks": [
    {
      ...
      "tmux_ask_target": "",
      "tmux_runner_target": "loom-qwen34b-CoQuant:0.0",
      "tmux_evaluator_target": "loom-qwen34b-CoQuant:0.1"
    }
  ]
}
```
...
- `meta.tmux_runner_target`: runner pane target, for example `loom-qwen34b-CoQuant:0.0`.
- `meta.tmux_evaluator_target`: evaluator pane target, for example `loom-qwen34b-CoQuant:0.1`.
...
```json
{
  ...
  "status": {
    ...
    "session": {
      ...
      "max_rounds": 200,
      "updated_at": "...",
      "tmux_session_name": "loom-qwen34b-CoQuant",
      "tmux_evaluator_target": "loom-qwen34b-CoQuant:0.1"
    }
  }
}
...
- `status.pid`: controller process id on the loom machine.
...
- Project: resolved id (`python3 …/cl_remote_api.py --print-project-id`) or explicit `LOOM_PROJECT_ID` / `LOOM_PROJECT_PATH`, plus task `<task>` / repo `<repo>`.
...
```text
loom/skills/remote_control/scripts/cl_remote_api.py
loom/skills/remote_control/scripts/cl_status.py
loom/skills/remote_control/scripts/cl_docs.sh
loom/skills/remote_control/scripts/cl_panes.py
loom/skills/remote_control/scripts/cl_tell_runner.py
```
...
```bash
export LOOM_BASE_URL="http://127.0.0.1:8765"
export LOOM_WEB_AUTH_TOKEN="optional-web-auth-token"
# Optional when several repos are registered:
# export LOOM_PROJECT_ID="<id>"   # or: export LOOM_PROJECT_PATH="/abs/path/to/repo"

python3 loom/skills/remote_control/scripts/cl_status.py
bash loom/skills/remote_control/scripts/cl_docs.sh <task-slug>
python3 loom/skills/remote_control/scripts/cl_panes.py <task-slug>
python3 loom/skills/remote_control/scripts/cl_tell_runner.py \
...
All snippets below use the same pattern as the `scripts/` tools: optional `LOOM_PROJECT_ID`, and `with_project()` on every `/api/project` and `/api/tasks` URL path.
...
if not tasks:
    raise SystemExit("no loom tasks found")
...
Only do this when the user explicitly asks OpenClaw to control loom.
...
runner_target = "loom-qwen34b-CoQuant:0.0"
message = (
...
- "Call `GET /api/projects`, then check loom status for the latest task in that root. Read `PLAN.md`, worker status, and runner/evaluator panes. Do not send commands."
...
1. Call `GET /api/projects`; choose `defaultProjectId` or the path the user named; export `LOOM_PROJECT_ID` for scripts or append `?project=` on task URLs.
...
When loom sends a `worker-start` event:
...
When loom sends a `worker-stop` event:
...
1. Resolve project id (`GET /api/projects` or `LOOM_PROJECT_ID`), then the active task and repo.
...
- Say which **project** (`LOOM_PROJECT_ID` or path) and task/repo you inspected.
