...
# Resolves ?project= via scripts/cl_remote_api.py (GET /api/projects), unless you export LOOM_PROJECT_ID / LOOM_PROJECT_PATH.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE="${LOOM_BASE_URL:-http://127.0.0.1:8765}"
export LOOM_BASE_URL="$BASE"

AUTH_ARGS=()
if [[ -n "${LOOM_WEB_AUTH_TOKEN:-}" ]]; then
  AUTH_ARGS=(-H "Authorization: Bearer ${LOOM_WEB_AUTH_TOKEN}")
fi
