...
BASE = os.environ.get("CLAUDELOOP_BASE_URL", "http://127.0.0.1:8765").rstrip("/")
...
def resolve_project_id() -> str:
    ...
    Order: CLAUDELOOP_PROJECT_ID (must exist), CLAUDELOOP_PROJECT_PATH (resolved path match),
    ...
    explicit = os.environ.get("CLAUDELOOP_PROJECT_ID", "").strip()
    path_hint = os.environ.get("CLAUDELOOP_PROJECT_PATH", "").strip()
    data = fetch_projects()
    ...
    if explicit:
        if explicit not in by_id:
            ...
            raise SystemExit(
                f"CLAUDELOOP_PROJECT_ID={explicit!r} is not registered. "
    ...
    if path_hint:
        try:
            ...
            raise SystemExit(f"CLAUDELOOP_PROJECT_PATH invalid: {exc}") from exc
        for p in projects:
            try:
            ...
            if rp == want:
                ...
                return _cached_project_id
        raise SystemExit(
            f"CLAUDELOOP_PROJECT_PATH={path_hint!r} did not match any entry in GET {BASE}/api/projects"
        )
    ...
    raise SystemExit(
        "Multiple projects registered; set CLAUDELOOP_PROJECT_ID or CLAUDELOOP_PROJECT_PATH. "
        f"Available: {lines}"
    )
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="claudeloop remote_control helpers")
    ap.add_argument(
        "--print-project-id",
        action="store_true",
        help="Print resolved project id for shell (uses CLAUDELOOP_* env)",
    )
    args = ap.parse_args()
