#!/usr/bin/env python3
"""Print latest claudeloop task, markdown summary, and worker status.

Project scope is resolved automatically via GET /api/projects unless you set:
  CLAUDELOOP_PROJECT_ID       — registry id
  CLAUDELOOP_PROJECT_PATH     — absolute path matching a registered project root
...
def main() -> None:
    ...
    if not tasks:
        raise SystemExit("no claudeloop tasks found for resolved project")
